...
void main() {
  group('EscalationEngine', () {
    ...
    test('returns 0.0 when totalPlayers is 0', () {
      final result =
          EscalationEngine.calculateBoldnessDelta(3, 0, 'safe');
      expect(result, 0.0);
    });
    ...
    test('calculates delta for all "I have" in safe tone', () {
      ...
      final result =
          EscalationEngine.calculateBoldnessDelta(4, 4, 'safe');
      expect(result, 0.5);
    });
    ...
    test('calculates delta for all "I have not" in deeper tone', () {
      ...
      final result =
          EscalationEngine.calculateBoldnessDelta(0, 5, 'deeper');
      expect(result, 0.0);
    });
    ...
    test('calculates delta for mixed answers in secretive tone', () {
      ...
      final result =
          EscalationEngine.calculateBoldnessDelta(3, 5, 'secretive');
    ...
    test('calculates delta for freaky tone', () {
      ...
      final result =
          EscalationEngine.calculateBoldnessDelta(2, 4, 'freaky');
      expect(result, 1.0);
    });
    ...
    test('updates boldness with EMA formula', () {
      ...
      final result =
          EscalationEngine.updateBoldnessScore(0.4, 0.8);
      expect(result, closeTo(0.52, 0.001));
    });

    test('clamps boldness to 0.0', () {
      final result =
          EscalationEngine.updateBoldnessScore(0.0, 0.0);
      expect(result, 0.0);
    });

    test('clamps boldness to 1.0', () {
      final result =
          EscalationEngine.updateBoldnessScore(1.0, 2.0);
      expect(result, 1.0);
    });
    ...
    test('returns 0.0 for round 0', () {
      expect(
          EscalationEngine.calculateProgressionModifier(0, 20), 0.0);
    });

    test('calculates mid-game progression', () {
      // 10/20 * 0.4 = 0.2, min(0.2, 0.2) = 0.2
      expect(EscalationEngine.calculateProgressionModifier(10, 20),
          closeTo(0.2, 0.001));
    });

    test('caps at 0.2 for late game', () {
      expect(EscalationEngine.calculateProgressionModifier(18, 20),
          closeTo(0.2, 0.001));
    });

    test('returns 0.0 when maxRounds is 0', () {
      expect(
          EscalationEngine.calculateProgressionModifier(5, 0), 0.0);
    });
    ...
    test('returns secretive for score >= 0.55', () {
      expect(
          EscalationEngine.determineTone(0.6, false), 'secretive');
    });

    test('returns secretive when score >= 0.8 but nsfw disabled', () {
      expect(
          EscalationEngine.determineTone(0.85, false), 'secretive');
    });
    ...
    test('freaky range capped at 7 when nsfw disabled', () {
      final r =
          EscalationEngine.getIntensityRange('freaky', false);
      expect(r.min, 7);
      expect(r.max, 7);
    });
    ...
    test('does not trigger with fewer than 2 rounds', () {
      ...
      expect(
          EscalationEngine.applyDeEscalation(0.6, rounds), 0.6);
    });

    test('triggers when last 2 rounds have > 75% "I have not" and intensity > 5',
        () {
      final rounds = [
        const OfflineRound(
          roundNumber: 1,
      ...
      expect(EscalationEngine.applyDeEscalation(0.6, rounds),
          closeTo(0.45, 0.001));
    });
